...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
	...
	func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
		...
		// DI Container
		let container = AppDependencyInject.rootContainer
		
		// HomeViewController
		let homeViewDependency = container.resolve(HomeViewReactorDependencyType.self)!
		let homeViewReaator = HomeViewReactor(dependency: homeViewDependency)
		let homeViewController = HomeViewController(reactor: homeViewReaator, container: container)
		...
		let favoriteViewDependency = container.resolve(FavoriteViewReactorDependencyType.self)!
		let favoriteViewReactor = FavoriteViewReactor(dependency: favoriteViewDependency)
		let favoriteViewController = FavoriteViewController(reactor: favoriteViewReactor)
