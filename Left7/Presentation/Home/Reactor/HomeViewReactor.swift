...
protocol HomeViewReactorDependencyType: AnyObject { // 추상화 (의존성 역전을 통한 의존성 분리)
	var useCase: HomeUseCaseType { get }
}

final class HomeViewReactorDependency: HomeViewReactorDependencyType { // 구현체
...
final class HomeViewReactor: Reactor {
	...
	private let dependency: HomeViewReactorDependencyType
	var initialState: State = State()
	
	//MARK: - Init
	
	init(dependency: HomeViewReactorDependencyType) {
	...
	func mutate(action: Action) -> Observable<Mutation> {
		switch action {
			case .fetchMovies:
				return dependency.useCase.fetchMovies(page: self.currentState.page)
			...
			case .loadNextPage:
				...
				return Observable.concat([
					...
					dependency.useCase.fetchMovies(page: self.currentState.page + 1)
...
private extension HomeViewReactor {
	func toggleFavoriteState(previousState: State, index: Int) -> Movie {
		...
		dependency.useCase.updateFavoriteMovies(movie)
